...
def main(
    ...
    # Apply field renaming for consistency with evaluation code `run_search_and_evaluate_multithreaded`
    for i, sample in enumerate(test_samples):
        if QUESTION_FIELD in sample:
            sample["problem"] = sample.pop(QUESTION_FIELD)  # Prompt -> problem
        if ANSWER_FIELD in sample:
            sample["answer"] = sample.pop(ANSWER_FIELD)  # Answer -> answer

    logger.info(
        f"Applied field renaming: {QUESTION_FIELD} -> problem, {ANSWER_FIELD} -> answer"
    )
